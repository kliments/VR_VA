...
public class CoverflowPropertiesForPermissionMenu : MonoBehaviour {
    ...
    private AssignAuthorityOverObject _player;
    private AssignAuthorityOverObject.Rights rights;
    ...
    void Start () {
        ...
        _player = (AssignAuthorityOverObject)FindObjectOfType(typeof(AssignAuthorityOverObject));
        menusParent = transform;
        while (menusParent.name != "PermissionMenue")
        {
	...
	void Update () {
        if (isHovered)
        {
            menusParent.GetComponent<CoverflowScriptForPermissionMenu>().currentButton = gameObject;
        ...
        if (isHovered && !highlight.activeSelf)
        {
            ...
            //Set values of current permission here!!
            if (_player == null) _player = (AssignAuthorityOverObject)FindObjectOfType(typeof(AssignAuthorityOverObject));
            _player.rights = (AssignAuthorityOverObject.Rights)System.Enum.Parse(typeof(AssignAuthorityOverObject.Rights), name);
        }
